...
class PartialPathState:
    def __init__(self, cost_matrix, included_cities=None, lower_bound=0, depth=0):
        self.cost_matrix = cost_matrix
        self.included_cities = [0] if included_cities is None else included_cities
        self.lower_bound = lower_bound
        self.depth = depth

    @property
    def queue_key(self):
        return 1 * self.lower_bound - 350 * self.depth

    def __lt__(self, other):
        return self.queue_key < other.queue_key

    def copy(self):
        # Create a new instance of the class with the same constructor arguments
        new_instance = PartialPathState(
            self.cost_matrix.copy(),
            list(self.included_cities),
            self.lower_bound,
            self.depth
        )
        return new_instance
    ...
    def reduce(self):
        """
        create a reduced cost matrix from self.cost_matrix
        and update lower_bound accordingly
        """
        for i in range(len(self.cost_matrix)):
            min_cost = np.min(self.cost_matrix[i])
            if not np.isinf(min_cost) and min_cost != 0:
                self.lower_bound += min_cost
                self.cost_matrix[i] -= min_cost
    ...
    def select_and_reduce(self, row_index, col_index):
        """
        select an edge going out from row_index and into col_index,
        then reduce the resulting cost matrix
        """
        self.included_cities.append(col_index)
        self.lower_bound += self.cost_matrix[row_index][col_index]
        self.cost_matrix[row_index, :] = np.inf
        self.cost_matrix[:, col_index] = np.inf
